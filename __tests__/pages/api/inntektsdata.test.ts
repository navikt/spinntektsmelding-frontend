...
describe('API Route: /api/inntektsdata', () => {
  ...
  describe('Development environment', () => {
    beforeEach(async () => {
      ...
      vi.resetModules();
      const apiModule = await import('../../../pages/api/inntektsdata');
  ...
  describe('Production environment', () => {
    beforeEach(async () => {
      ...
      process.env.INNTEKTSDATA_API = '/inntektsdata';
      vi.resetModules();
      const apiModule = await import('../../../pages/api/inntektsdata');
      handler = apiModule.default;
      config = apiModule.config;
    ...
    it('should proxy request to backend', () => {
      ...
      expect(mockHttpProxyMiddleware).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          target: 'http://api.example.com/inntektsdata',
    ...
    it('should use correct proxy configuration', () => {
      ...
      expect(proxyConfig.target).toBe('http://api.example.com/inntektsdata');
    ...
    it('should rewrite path correctly', () => {
      ...
      expect(pathRewrite.patternStr).toBe('^/api/inntektsdata');
    ...
    it('should construct basePath from environment variables', async () => {
      ...
      process.env.INNTEKTSDATA_API = '/v1/orgnr';
  ...
  describe('Request handling', () => {
    beforeEach(async () => {
      process.env.NODE_ENV = 'production';
      process.env.IM_API_URI = 'api.example.com';
      process.env.INNTEKTSDATA_API = '/aktive-orgnr';
      vi.resetModules();
      const apiModule = await import('../../../pages/api/inntektsdata');
      handler = apiModule.default;
    });
    ...
    it('should handle requests with query parameters', () => {
      mockReq.url = '/api/inntektsdata?filter=active';

      handler(mockReq as NextApiRequest, mockRes as NextApiResponse);

      expect(mockHttpProxyMiddleware).toHaveBeenCalledWith(
        expect.objectContaining({
          url: '/api/inntektsdata?filter=active'
        }),
        mockRes,
        expect.any(Object)
      );
  ...
  describe('Proxy configuration details', () => {
    ...
    it('should remove /api/inntektsdata prefix from path', () => {
      handler(mockReq as NextApiRequest, mockRes as NextApiResponse);

      const proxyConfig = mockHttpProxyMiddleware.mock.calls[0][2];
      const rewrite = proxyConfig.pathRewrite[0];

      const testPath = '/api/inntektsdata/test';
