...
describe('API Route: /api/arbeidsgivere', () => {
  ...
  describe('Development environment', () => {
    beforeEach(async () => {
      ...
      vi.resetModules();
      const apiModule = await import('../../../pages/api/arbeidsgivere');
  ...
  describe('Production environment', () => {
    ...
    it('should rewrite path correctly', () => {
      ...
      expect(pathRewrite.patternStr).toBe('^/api/arbeidsgivere');
  ...
  describe('Request handling', () => {
    ...
    it('should handle requests with query parameters', () => {
      mockReq.url = '/api/arbeidsgivere?filter=active';

      handler(mockReq as NextApiRequest, mockRes as NextApiResponse);

      expect(mockHttpProxyMiddleware).toHaveBeenCalledWith(
        expect.objectContaining({
          url: '/api/arbeidsgivere?filter=active'
        }),
        mockRes,
        expect.any(Object)
      );
  ...
  describe('Proxy configuration details', () => {
    ...
    it('should remove /api/arbeidsgivere prefix from path', () => {
      handler(mockReq as NextApiRequest, mockRes as NextApiResponse);

      const proxyConfig = mockHttpProxyMiddleware.mock.calls[0][2];
      const rewrite = proxyConfig.pathRewrite[0];

      const testPath = '/api/arbeidsgivere/test';
