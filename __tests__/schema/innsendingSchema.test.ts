...
function hasErrorMessage(issues: z.ZodError['issues'][number][] | undefined, message: string): boolean {
  if (!issues) return false;
  return issues.some((issue) => {
    if (issue.message === message) return true;
    if ('errors' in issue && Array.isArray(issue.errors)) {
      return issue.errors.some((errorGroup: z.ZodError['issues'][number][]) =>
        errorGroup.some((err: z.ZodError['issues'][number]) => err.message === message)
      );
    }
    return false;
  });
...
describe('InnsendingSchema', () => {
  ...
  it('should validate InnsendingSchema with error when agp is short n redusertloennIAGP is missing', () => {
    const data = {
      agp: {
        perioder: [{ fom: '2023-02-17', tom: '2023-03-04' }],
        egenmeldinger: [],
  ...
  it('should validate InnsendingSchema with error when agp is short and redusertloennIAGP is missing', () => {
    const data = {
      agp: {
        perioder: [{ fom: '2023-02-17', tom: '2023-02-24' }],
        egenmeldinger: [],
        redusertLoennIAgp: {}
      },
      inntekt: {
